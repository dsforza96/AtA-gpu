...
void GPU_trans(double *A, double *B, double *C,
...
/*
  lda is the width in actual memory.
...
void ata(double *A, double *C,
  ...
  if (depth <= 1 || stop) {
    ...
    ata(A11, W_1, lda, ldc, XA2, YA2, depth - 1);  // S1 = ata(A11)
    ata(A21, W_2, lda, ldc, XA2, YA2, depth - 1);  // S2 = ata(A21)
    GPU_add(C11, A11, W_1, lw1, lda, lw1, XA2, YA2, 1.0, -1.0);  // C11 = S1 + S2
    ata(W_1, W_2, C11, lw1, lw2, ldc, XA2, XB2, XC2, YA2, YB2, YC2, depth - 1);  // S3 = ata(A12)
    ata(W_1, B22, C12, lw1, ldb, ldc, XA2, XB2, XC2, YA2, YB2, YC2, depth - 1);  // S4 = ata(A22)
    GPU_add(C22, C12, C12, ldc, ldc, ldc, XC2, YC2, 1.0,  1.0);  // C22 = S3 + S4
    GPU_trans(C11, C22, C22, ldc, ldc, ldc, XC2, YC2, 1.0,  1.0);  // A12_t
    strassen(A22, W_2, C21, lda, lw2, ldc, XA2, XB2, XC2, YA2, YB2, YC2, depth - 1);  // S5 = strassen(A12_t, A11)
    GPU_trans(C11, C22, C22, ldc, ldc, ldc, XC2, YC2, 1.0,  1.0);  // A22_t
    strassen(A12, B21, C11, lda, ldb, ldc, XA2, XB2, XC2, YA2, YB2, YC2, depth - 1);  // S6 = strassen(A22_t, A21)
    GPU_add(W_1, C11, C11, lw1, ldc, ldc, XC2, YC2, 1.0,  1.0);  // C21 = S5 + S6
  }
  cudaFree(W_1);
  cudaFree(W_2);
